...
d3.csv('data.csv', function (d) {
    ...
    chart.renderSlices = function () {
        ...
        function arcTween(d) {
            ...
            const interpolate = d3.interpolate(currentArc, d) // d3.interpolate - 生成一个任意数值的插值器.
    ...
    chart.renderText = function () {
        ...
        const labels = d3.select('.pie').selectAll('.label').data(chart.arcAngle(data))
    ...
    chart.addMouseOn = function () {
        ...
        d3.selectAll('.arc')
            .on('mouseover', function (d, i) {
                const e = d
                d3.select(e.target).attr('d', arcLarger(i))
            })
            .on('mouseleave', function (d, i) {
                const e = d
                d3.select(e.target).attr('d', arcNormal(i))
            })
    }
